        ...
        if (address1 !== "" && address2 !== "") {
            $.ajax({
                ...
                if (response.status === "NOT_FOUND") {
                    ...
                    var duration = parseInt(response.routes[0].legs[0].duration.text) / 2;
                    var stepTime = parseInt(response.routes[0].legs[0].steps[0].duration.text);
                    var j = 0;
                    console.log(duration);
                    console.log(parseInt(stepTime));
                    while (stepTime < duration) {
                        j++;
                        stepTime += parseInt(response.routes[0].legs[0].steps[j].duration.text);
                        console.log(j);
                        console.log(stepTime);
                    }
                    console.log("lat" + response.routes[0].legs[0].steps[j].end_location.lat);
                    console.log("lng" + response.routes[0].legs[0].steps[j].end_location.lng);
